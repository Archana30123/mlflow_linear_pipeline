...
```
## Docker logs
## View logs (foreground run prints logs to terminal; for detached):
```
docker logs -f ml-predict-api
```

## Restart (to pick up a new exported_model after retraining):
```
docker restart ml-predict-api
```

## Stop & remove:
```
docker stop ml-predict-api
docker rm ml-predict-api   # only if not using --rm
```
